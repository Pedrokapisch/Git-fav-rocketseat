import { GithubUser } from "./GitUsers.js"
...
export class Favorites {
  ...
  async add(username) {
    try {
      ...
      const user = await GithubUser.search(lowerCaseUsername)      
      if(user.login === undefined ) {
        throw new Error('Usuário não encontrado! Verifique a ortografia e tente novamente')
      }
...
export class FavoritesView extends Favorites {
  ...
  noUser() {
    if (this.tbody.querySelector('tr') === null || this.tbody.querySelector('tr') != emptyRow) {
      this.removeAllTr()
      const emptyRow = document.createElement('tr')
      emptyRow.innerHTML = `
                  <tr class="no-users">
              <td class="no-users" colspan="4">
                <div class="td-wrapper">
                  ...
                  </svg><span>Nenhum favorito ainda</span>
                </div>
              </td>
            </tr>
      `
  ...
  createRow() {
    ...
    <tr>
      <td class="user">
        <img src="https://github.com/Pedrokapisch.png" alt="Imagem de Pedrokapisch">
        <a href="https://github.com/Pedrokapisch" target="_blank">
          <p>Pedro Augusto Kapisch</p>
          <span>pedrokapisch</span>
        </a>
      </td>
